...
async function seed() {
  if (!usePostgres) {
    // Garante que a pasta existe apenas para SQLite
    const dir = path.dirname(path.resolve(DB_PATH));
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  }

  const dsOptions: any = usePostgres
    ? {
        type: 'postgres',
  ...
  await ds.initialize();
  console.log(`🗄️  Banco ${usePostgres ? 'PostgreSQL' : 'SQLite'} pronto${usePostgres ? '' : ' em: ' + DB_PATH}`);
